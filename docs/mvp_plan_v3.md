...
5. **Local-first competition plumbing** — installs may opt in (off by default, plain-language toggle) to share only **anonymized aggregates** to a benchmark service; raw events, content, and visitor data never leave the install. K-anonymity guardrail: aggregates are published only when the sample is ≥10 readers. A local `share_ledger` records everything sent. The MVP ships the plumbing plus one "network percentile" line in the dashboard; the full game is post-G2.
...
- **Benchmark service** — small, AGPL, self-hostable by anyone, with a documented aggregate-only protocol (a seed of the federation/discovery layer). It ingests only anonymized summaries and serves percentiles/rankings back. It never sees raw events.
...
- **Benchmark service:** AGPL-3.0 with a self-hostable, documented aggregate-only protocol — the platform's own collector must never become the closed super-instance.
